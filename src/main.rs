...
fn main() {
    ...
    let mut files_to_process: Vec<(PathBuf, Language, u64)> =
        Vec::with_capacity(files_to_analyze.len());
...
fn output_table(
    summary: &Summary,
    file_details: &[(PathBuf, Language, FileStats)],
    show_files: bool,
) {
    ...
    if show_files && !file_details.is_empty() {
        ...
        for (path, lang, stats) in sorted {
            let display_path = if path.as_os_str().len() > 48 {
                format!(
                    "...{}",
                    &path.to_string_lossy()[path.to_string_lossy().len() - 45..]
                )
            } else {
